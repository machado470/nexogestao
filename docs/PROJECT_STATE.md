...
- Service Orders e WhatsApp preservam facades públicas, com leituras tenant-scoped extraídas para serviços focados conforme o ADR 0004; as dependências de leitura agora são obrigatoriamente fornecidas pelo container Nest, sem fallback de construção manual, e um guardrail estrutural protege essa composição. Transações, claims, Timeline, Outbox e autoridade Finance permanecem nas unidades originais e cobertas pelos testes de caracterização existentes.
