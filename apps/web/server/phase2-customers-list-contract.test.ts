...
describe("Phase 2 customers.list input contract", () => {
  ...
  it.each([
    ...
    [
      ...
      { page: 2, limit: 300, search: "Cliente" },
      "?page=2&limit=300&search=Cliente",
    ],
    ...
    await expect(caller.customers.list(input as any)).resolves.toEqual(
      response.data
    );
    expect(String(fetchMock.mock.calls[0][0])).toContain(
      `/customers${expectedQuery}`
    );
  });
  ...
  it.each([
    ...
    "rejeita autoridade tenant e chaves extras antes do upstream: %j",
    async input => {
  ...
  it("mantém o alias nexo.* no mesmo contrato estrito", async () => {
    ...
    await expect(
      caller.nexo.customers.list({ orgId: "spoofed" } as any)
    ).rejects.toBeTruthy();
    expect(fetchMock).not.toHaveBeenCalled();
  });
});
